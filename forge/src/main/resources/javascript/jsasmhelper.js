...
function setupJSASMHelper (){
    ...
    function QueryConstructor(clazz) {
        ...
        this.method = function(method) {
            this._method = ASMAPI.mapMethod(method);
            return this;
        }
    ...
    function outputNodeInstructions(methodNode) {
        eachInstruction(methodNode, function (instruction) {
            ...
            printInsn(instruction, []);
            /*printIfDefined("Type", instruction);
            printIfDefined("Opcode", OPCODES[instruction.getOpcode()]);
    ...
    /**
     * Replaces all the . with / so you can use a direct class path easier
     * @return QueryConstructor
     */
    function createQueryConstructorWithClass(clazz) {
        return new QueryConstructor(clazz.replaceAll('\\.','/'))
    }
