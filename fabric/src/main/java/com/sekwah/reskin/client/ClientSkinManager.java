...
public class ClientSkinManager {
    ...
    public static void checkSkin(AbstractClientPlayer player) {
        ...
        ResourceLocation currentSkin = playerInfoAccessor.getTextureLocations().get(MinecraftProfileTexture.Type.SKIN);
        ...
        if(wantedSkin != null) {
            if(currentSkin != wantedSkin.resourceLocation) {
                if(!originalSkinMap.containsKey(player.getUUID())) {
                    originalSkinMap.put(player.getUUID(), new ClientSkinData(currentSkin, playerInfoAccessor.getSkinModel()));
                }
                playerInfoAccessor.getTextureLocations().put(MinecraftProfileTexture.Type.SKIN, wantedSkin.resourceLocation);
            }

            playerInfoAccessor.setSkinModel(wantedSkin.modelType);
        }
    }
}
